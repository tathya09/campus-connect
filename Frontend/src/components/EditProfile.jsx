...
const EditProfile = () => {
    ...
    const editProfileHandler = async () => {
        ...
        // Only append profilePicture if it's a File object (not a URL string)
        if (input.profilePhoto && input.profilePhoto instanceof File) {
            formData.append("profilePicture", input.profilePhoto);
        }
        try {
            setLoading(true);
            const res = await axios.post('https://campus-connect-bn54.vercel.app/api/v1/user/profile/edit', formData, {
                headers: {
