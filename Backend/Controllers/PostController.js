...
export const getAllPost = async (req, res) => {
    try {
        const posts = await Post.find().sort({ createdAt: -1 })
            .populate({ path: 'author', select: 'username profilePicture usertype' })
            .populate({
                path: 'comments',
                sort: { createdAt: -1 },
                populate: {
...
export const likePost = async (req, res) => {
    try {
        ...
        if (!post) return res.status(404).json({ message: 'Have not Posted yet', success: false });
...
export const getCommentsOfPost = async (req,res) => {
    try {
        ...
        const comments = await Comment.find({post:postId}).populate('author', 'username, profilePicture ,usertype');
